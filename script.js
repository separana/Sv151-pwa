...
// ==== Live price refresh (TCGplayer via pokemontcg.io) ====
const PRICE_CACHE_KEY='sv151-live-cards', PRICE_CACHE_TS_KEY='sv151-live-ts', ONE_DAY=24*3600*1000;
// const API_KEY=''; // optioneel (frontend zichtbaar). Liever proxy als je key wil.
...
async function ensureFreshPrices(force=false){
  try{
    ...
    if(!force && !stale && cached.length){
      CARDS=cached;
      localStorage.setItem('sv151-cards-cache',JSON.stringify(CARDS));
      populateFilters(); applyFilters();
      statusEl.textContent=`Live prijzen (cache <24u) • ${CARDS.length} kaarten`;
      return;
    }
    ...
    console.error(e);
    statusEl.textContent='Live prijsupdate mislukt (gebruik cache).';
  }
}
...
function renderGrid(){
  ...
  FILTERED.forEach((c,idx)=>{
    ...
    card.addEventListener('click',()=>{OWNED[c.id]=!OWNED[c.id]; if(OWNED[c.id]&&!QTY[c.id])QTY[c.id]=1; if(!OWNED[c.id]){delete QTY[c.id];} saveAll(); applyFilters();});
    // dblclick to open viewer
...
// Viewer + historiek (client-side logging)
...
(function init(){
  ...
  ensureFreshPrices(); // live prijzen of <24u cache
})();
