...
let OWNED_REV=JSON.parse(localStorage.getItem('owned_rev')||'{}');
function saveAll(){localStorage.setItem(STORAGE_KEY,JSON.stringify({owned:OWNED})); localStorage.setItem(WISH_KEY,JSON.stringify(WISH)); localStorage.setItem(QTY_KEY,JSON.stringify(QTY)); localStorage.setItem('owned_rev',JSON.stringify(OWNED_REV));}
...
statusEl=document.getElementById('status'), ownedValue=document.getElementById('ownedValue'),
...
async function fetchCardsLive(){
  ...
  return (data.data||[]).map(c=>{
    c.hasReverse = !!(c.tcgplayer && c.tcgplayer.prices && c.tcgplayer.prices.reverseHolofoil);
    return c;
  });
}
async function ensureFreshPrices(force=false){
  try{
    ...
    if(!force && !stale && cached.length){
      CARDS=cached; populateFilters(); applyFilters(); statusEl.textContent=`Live prijzen (cache <24u) • ${CARDS.length} kaarten`; return;
    }
    ...
    statusEl.textContent=`Live prijzen geüpdatet • ${CARDS.length} kaarten`; logHistorySample();
...
function pickPrice(c,src){
  if(src==='tcgplayer'){
    const tp=c.tcgplayer?.prices; if(tp){
      const usd=[tp.holofoil?.market,tp.normal?.market,tp.reverseHolofoil?.market,tp.ultraRare?.market,tp.rainbowRare?.market,tp.gold?.market].filter(v=>typeof v==='number'&&v>0);
      if(usd.length) return +(Math.max(...usd)*USD_TO_EUR).toFixed(2);
    } return null;
  } else {
    const cm=c.cardmarket?.prices; if(cm){
      const arr=[cm.trendPrice,cm.avg7,cm.avg30,cm.averageSellPrice,cm.lowPrice,cm.suggestedPrice,cm.reverseHoloTrend,cm.reverseHoloSell].filter(v=>typeof v==='number'&&v>0);
...
function applyFilters(){
  ...
  renderGrid(); updateStats(); updateProgressBars();
}
function renderGrid(){
  grid.innerHTML='';
  FILTERED.forEach((c,idx)=>{
    ...
    // reverse chip (masterset)
    if(c.hasReverse){
      const rev=document.createElement('div');
      rev.className='rev-chip'+(OWNED_REV[c.id]?' active':'');
      rev.textContent='R';
      rev.title=OWNED_REV[c.id]?'Reverse: heb ik':'Reverse: toevoegen';
      rev.onclick=(e)=>{e.stopPropagation(); if(OWNED_REV[c.id]) delete OWNED_REV[c.id]; else OWNED_REV[c.id]=true; saveAll(); rev.classList.toggle('active'); updateProgressBars();};
      card.appendChild(rev);
    }

    // gestures
    let timer=null;
    card.addEventListener('touchstart',()=>{timer=setTimeout(()=>{QTY[c.id]=(QTY[c.id]||(OWNED[c.id]?1:0))+1; OWNED[c.id]=true; saveAll(); applyFilters();},500)});
    card.addEventListener('touchend',()=>clearTimeout(timer));
    card.addEventListener('click',()=>{OWNED[c.id]=!OWNED[c.id]; if(OWNED[c.id]&&!QTY[c.id])QTY[c.id]=1; if(!OWNED[c.id]){delete QTY[c.id]; delete WISH[c.id];} saveAll(); applyFilters();});
...
function updateProgressBars(){
  const totalUnique = CARDS.length||207;
  const ownedUnique = Object.keys(OWNED).filter(id=>OWNED[id]).length;
  const totalMaster = totalUnique + CARDS.filter(c=>c.hasReverse).length;
  const ownedMaster = ownedUnique + Object.keys(OWNED_REV).filter(id=>OWNED_REV[id]).length;
  const pctSet = Math.round((ownedUnique/totalUnique)*100)||0;
  const pctMaster = Math.round((ownedMaster/totalMaster)*100)||0;
  document.getElementById('setPct').textContent = pctSet+'%';
  document.getElementById('masterPct').textContent = pctMaster+'%';
  document.getElementById('setBar').style.width = pctSet+'%';
  document.getElementById('masterBar').style.width = pctMaster+'%';
}

// viewer + history (light)
...
function drawHistory(cardId){const cvs=document.getElementById('priceChart'), ctx=cvs.getContext('2d'); cvs.width=600; cvs.height=160; ctx.clearRect(0,0,cvs.width,cvs.height); const samples=(HISTORY[cardId]||[]).slice(-40); const data=samples.map(s=>s['tp']).filter(v=>typeof v==='number'); if(!data.length){ctx.fillStyle='#9ab0c7'; ctx.fillText('Nog geen data — prijzen bouwen lokaal op.',16,24); return;} const min=Math.min(...data), max=Math.max(...data); const pad=10, W=cvs.width-2*pad, H=cvs.height-2*pad; ctx.strokeStyle='#eacb6b'; ctx.lineWidth=2; ctx.beginPath(); data.forEach((v,i)=>{const x=pad+(i*(W/(data.length-1||1))); const y=pad+H-((v-min)/(max-min||1))*H; if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y);}); ctx.stroke(); ctx.fillStyle='#9ab0c7'; ctx.fillText('Min: €'+min.toFixed(2)+'  Max: €'+max.toFixed(2), pad, cvs.height-8);}
function logHistorySample(){const t=Date.now(); for(const c of CARDS){const tp=pickPrice(c,'tcgplayer'); if(!HISTORY[c.id]) HISTORY[c.id]=[]; HISTORY[c.id].push({t,tp}); if(HISTORY[c.id].length>120) HISTORY[c.id].shift();} localStorage.setItem(HISTORY_KEY, JSON.stringify(HISTORY));}
...
// Init
(function init(){
  try{const cache=JSON.parse(localStorage.getItem('sv151-cards-cache')||'[]'); if(cache.length){CARDS=cache; populateFilters(); applyFilters(); statusEl.textContent=`Cache: ${CARDS.length} kaarten`;}}catch{}
  setTabs();
  ensureFreshPrices();
})();
